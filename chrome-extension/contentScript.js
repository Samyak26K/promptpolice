...
const GENERIC_FALLBACK_STRATEGY = {
  user: [
    ".user-message",
...
function detectByTurnSelector(turnSelector) {
  if (!turnSelector) {
    return null;
  }
  ...
  for (let index = turns.length - 1; index >= 0; index -= 1) {
    ...
    const assistantNode = turn.querySelector("[data-message-author-role='assistant'], [data-testid='assistant-message'], .markdown, .prose");
...
function buildConversationSnapshot() {
  const chatgptLists = buildStructuredLists(CHATGPT_STRATEGY);
  const claudeLists = buildStructuredLists(CLAUDE_STRATEGY);
  const genericLists = buildStructuredLists(GENERIC_FALLBACK_STRATEGY);

  const usersTotal = chatgptLists.userNodes.length + claudeLists.userNodes.length + genericLists.userNodes.length;
  const assistantsTotal = chatgptLists.assistantNodes.length + claudeLists.assistantNodes.length + genericLists.assistantNodes.length;

  console.log("SafeNet Debug:", {
  ...
  const byTurn = detectByTurnSelector(CHATGPT_STRATEGY.turn) || detectByTurnSelector(CLAUDE_STRATEGY.turn);
  const byChatgptLists = findPairFromLists(chatgptLists.userNodes, chatgptLists.assistantNodes);
  const byClaudeLists = findPairFromLists(claudeLists.userNodes, claudeLists.assistantNodes);
  const byGenericLists = findPairFromLists(genericLists.userNodes, genericLists.assistantNodes);

  const candidate = [byTurn, byChatgptLists, byClaudeLists, byGenericLists].find((item) => item && (item.prompt || item.response));
  const fallback = candidate ? null : getFallbackSnapshot();
  const selected = candidate || fallback || { prompt: "", response: "", userNode: null, assistantNode: null };
